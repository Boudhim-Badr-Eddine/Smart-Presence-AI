...
import { MessageSquare, Send, CheckCircle, AlertCircle, Clock, ThumbsUp, ThumbsDown, Tag, ChevronRight } from "lucide-react";
  ...
  return (
    <RoleGuard allow={["student"]}>
      <div className="mx-auto max-w-7xl p-6">
        <Breadcrumbs items={[{ label: "Espace Étudiant", href: "/student" }, { label: "Feedback" }]} />

        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white dark:text-white light:text-gray-900">Retour sur la formation</h1>
            <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Exprimez vos retours pour améliorer l'expérience.</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-6">
          {[
            ...
            <div key={card.label} className={`rounded-lg border border-white/10 p-4 ${card.color} dark:border-white/10 light:border-gray-200 light:bg-gray-50`}>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div>
                <h2 className="text-lg font-semibold text-white dark:text-white light:text-gray-900">Partager un retour</h2>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-2 text-sm font-medium text-white dark:text-white light:text-gray-900 block">
                ...
                <select
                  value={form.category}
                  onChange={(e) => setForm({ ...form, category: e.target.value as FeedbackItem["category"] })}
                  className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white dark:border-white/10 dark:bg-white/5 dark:text-white light:border-gray-200 light:bg-gray-50 light:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
              ...
              <label className="space-y-2 text-sm font-medium text-white dark:text-white light:text-gray-900 block">
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, sentiment: e.target.value as FeedbackItem["sentiment"] })}
                  className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white dark:border-white/10 dark:bg-white/5 dark:text-white light:border-gray-200 light:bg-gray-50 light:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-white dark:text-white light:text-gray-900">Suivi des retours</h2>
                <p className="text-xs text-zinc-400">Historique des échanges avec l'équipe pédagogique.</p>
              </div>
              ...
              {feedback?.map((item) => (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="font-semibold text-white dark:text-white light:text-gray-900">{item.title}</h3>
                          <span className={`rounded-full border px-2 py-0.5 text-xs ${badgeStyles[item.status]}`}>
                        ...
                        <p className="text-sm text-zinc-300 dark:text-zinc-300 light:text-gray-700 mt-1">{item.response ?? "Feedback reçu, en attente de réponse."}</p>
                    ...
                    {item.status === "resolu" && <CheckCircle className="h-5 w-5 text-emerald-300 flex-shrink-0" />}
                  </div>
                </motion.div>
              ))}
