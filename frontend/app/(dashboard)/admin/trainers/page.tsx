...
import AdvancedFilterDrawer, { FilterField, FilterValues } from "@/components/ui/AdvancedFilterDrawer";
...
const TrainerCreateEditModal = nextDynamic(() => import("@/components/modals/TrainerCreateEditModal"), {
  ssr: false,
});
...
export default function AdminTrainersPage() {
  ...
  const updateCachedLists = (updater: (items: Trainer[]) => Trainer[]) => {
    const queries = queryClient.getQueriesData<{ items: Trainer[]; total_pages: number }>({ queryKey: ["admin-trainers"] });
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newTrainer) => {
      ...
      updateCachedLists((items) => [
        { id: Date.now(), name: newTrainer.name, email: newTrainer.email, subjects: newTrainer.subjects },
        ...items,
      ]);
    },
    onSuccess: () => {
  ...
  const tourSteps: TourStep[] = [
    ...
    },
    {
      ...
      content: "Utilisez la barre de recherche pour filtrer les formateurs par nom, email ou matière.",
      placement: "bottom",
    },
    {
      target: "[data-tour='filters']",
      title: "Filtres avancés",
      content: "Accédez aux filtres avancés pour des recherches plus précises et sauvegardez vos filtres favoris.",
      placement: "left",
    },
  ];
      ...
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-blue-500/15 bg-gradient-to-br from-blue-500/10 via-zinc-950 to-black p-6">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-white/70">Pilotez la disponibilité des formateurs, leurs matières et la capacité de sessions.</p>
            </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-4 shadow-sm">
          ...
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <div data-tour="search" className="flex flex-1 items-center gap-2 rounded-lg border border-white/10 bg-zinc-950 px-3 py-2">
              <input
                value={search}
                onChange={(e) => {
            ...
            <div className="overflow-hidden rounded-lg border border-white/10">
              <table className="w-full">
                <thead className="border-b border-white/10 bg-white/5">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase text-white/60">Nom</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase text-white/60">Email</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase text-white/60">Matière</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase text-white/60">Actions</th>
                  </tr>
                </thead>
                <tbody>
