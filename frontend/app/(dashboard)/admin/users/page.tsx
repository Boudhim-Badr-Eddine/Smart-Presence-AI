...
export default function AdminUsersPage() {
  ...
  const handleSave = async () => {
    ...
    if (!apiBase) {
      setMessage({ type: "error", text: "API non configurée (NEXT_PUBLIC_API_BASE_URL manquant)." })
      return
    }

    const token = typeof window !== "undefined" ? localStorage.getItem("spa_access_token") : null
    if (!token) {
      setMessage({ type: "error", text: "Session manquante. Reconnectez-vous avant de créer un compte." })
      return
    }
      ...
      <div className="mx-auto max-w-5xl rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl backdrop-blur-xl space-y-4">
        <div className="flex flex-col gap-2 rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white/80">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <span className={`rounded-full px-3 py-1 border ${envConfigured ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-200" : "border-amber-500/40 bg-amber-500/15 text-amber-200"}`}>
              API base: {envConfigured ? "OK" : "Manquante (NEXT_PUBLIC_API_BASE_URL)"}
            </span>
            <span className={`rounded-full px-3 py-1 border ${hasToken ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-200" : "border-amber-500/40 bg-amber-500/15 text-amber-200"}`}>
              Jeton session: {hasToken ? "OK" : "Absent (connectez-vous)"}
            </span>
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-white/80">3 photos requises</span>
          </div>
        </div>
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">Créer un nouveau compte utilisateur avec enrôlement facial</p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div className={`h-1 flex-1 rounded-full ${enrollmentStep === "facial" ? "bg-amber-500" : "bg-white/10"}`} />
          <div className={`flex h-10 w-10 items-center justify-center rounded-full font-semibold ${
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 space-y-6">
            ...
            {!isFormValid() && (
              <div className="flex items-start gap-2 rounded-lg bg-amber-600/20 p-3 text-sm text-amber-400">
                ...
                <p>Veuillez remplir tous les champs obligatoires (*) et vous assurer que le mot de passe contient au moins 6 caractères.</p>
              </div>
            )}
          </div>
        )}
        ...
        {enrollmentStep === "facial" && (
          <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-zinc-400 mb-4">Utilisateur: <span className="font-semibold text-amber-400">{formData.firstName} {formData.lastName}</span></p>
            </div>

            <div>
              <div className="mb-4">
                <h3 className="text-sm font-semibold text-white mb-2">Capture de photos ({faceImages.length}/3)</h3>
                <p className="text-xs text-white/60 mb-2">Capturez 3 angles: gauche, face, droite</p>
              ...
              {showWebcam && (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={capturePhoto} disabled={faceImages.length >= 3} className="flex-1 bg-amber-500 hover:bg-amber-600">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                    {faceImages.map((image, index) => (
                      <div key={index} className="relative group overflow-hidden rounded-lg border border-white/10 h-24">
                        <Image src={image} alt={`Face ${index + 1}`} fill className="object-cover" loading="lazy" />
                  ...
                  {faceImages.length < 3 && (
                    <div className="flex items-start gap-2 rounded-lg bg-amber-600/20 p-3 text-sm text-amber-400">
                      <AlertCircle className="h-4 w-4 mt-0.5 flex-shrink-0" />
                      <p>Vous devez capturer au moins {3 - faceImages.length} photo(s) supplémentaire(s)</p>
                    </div>
                  )}
