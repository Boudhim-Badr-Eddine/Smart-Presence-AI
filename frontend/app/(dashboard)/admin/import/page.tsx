  ...
  return (
    <RoleGuard allow={["admin"]}>
      <div className="mx-auto max-w-7xl p-6">
        <Breadcrumbs items={[{ label: "Administration", href: "/admin" }, { label: "Importer Données" }]} />
        
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-white dark:text-white light:text-gray-900">Importer Données</h1>
          <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Importer des étudiants, formateurs, classes et sessions via CSV/Excel</p>
        </div>

        <div className="grid gap-6 md:grid-cols-2 mb-8">
          {/* Templates Download */}
          <div className="rounded-lg border border-white/10 bg-white/5 p-6 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white" data-tour-id="import-templates">
            ...
            <div className="space-y-2">
              {[
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-medium text-white dark:text-white light:text-gray-900">{template.label}</p>
                    <p className="text-xs text-zinc-400 dark:text-zinc-400 light:text-gray-600">Format standard</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-6 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
            ...
            <div className="mb-4">
              <label className="text-xs uppercase tracking-wide text-zinc-400 dark:text-zinc-400 light:text-gray-600">Type de données</label>
              <select
                value={entity}
            ...
            >
              ...
              <p className="text-sm font-medium text-white dark:text-white light:text-gray-900">Déposer le fichier ici</p>
              <p className="text-xs text-zinc-400 dark:text-zinc-400 light:text-gray-600 mt-1">ou cliquer pour sélectionner</p>
              {selectedFile && (
                <p className="text-xs text-emerald-300 mt-3">{selectedFile.name}</p>
              )}
            </div>
        ...
        {importStatus && (
          <div className="mb-8 rounded-lg border border-emerald-600/20 bg-emerald-600/10 p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white dark:text-white light:text-gray-900 mb-2">Importation Réussie</h3>
                <div className="grid gap-2 md:grid-cols-2">
                  <div className="rounded-lg bg-white/5 dark:bg-white/5 light:bg-white p-3">
                    <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Enregistrements importés</p>
                  ...
                  {importStatus.errors > 0 && (
                    <div className="rounded-lg bg-red-600/10 dark:bg-red-600/10 light:bg-red-50 p-3">
                      <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Erreurs</p>
        ...
        <div className="rounded-lg border border-white/10 bg-white/5 p-6 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
          ...
          <ul className="space-y-3 text-sm">
            <li className="flex gap-3">
              ...
              <span className="text-zinc-300 dark:text-zinc-300 light:text-gray-700">Téléchargez les modèles de fichiers correspondant à vos données</span>
            </li>
            <li className="flex gap-3">
              <span className="text-amber-300 font-semibold">2.</span>
              <span className="text-zinc-300 dark:text-zinc-300 light:text-gray-700">Remplissez les fichiers avec vos données en respectant le format</span>
            </li>
            <li className="flex gap-3">
              <span className="text-amber-300 font-semibold">3.</span>
              <span className="text-zinc-300 dark:text-zinc-300 light:text-gray-700">Déposez le fichier CSV/Excel dans la zone de téléchargement</span>
            </li>
            <li className="flex gap-3">
              <span className="text-amber-300 font-semibold">4.</span>
              <span className="text-zinc-300 dark:text-zinc-300 light:text-gray-700">Vérifiez les résultats et les erreurs (le cas échéant)</span>
            </li>
            <li className="flex gap-3">
              <span className="text-amber-300 font-semibold">5.</span>
              <span className="text-zinc-300 dark:text-zinc-300 light:text-gray-700">Les données seront validées et intégrées au système</span>
            </li>
          </ul>
        </div>
        ...
        <OnboardingTour
          tourId="import"
          steps={[
            { target: "[data-tour-id='import-templates']", title: "Modèles prêts", content: "Téléchargez les CSV/Excel standardisés.", placement: "right" },
            { target: "[data-tour-id='import-drop']", title: "Déposez votre fichier", content: "Glissez ou cliquez pour charger le fichier à importer.", placement: "bottom" },
            { target: "[data-tour-id='import-submit']", title: "Lancer l'import", content: "Suivez la progression en temps réel (WebSocket).", placement: "right" },
          ]}
          autoStart
        />
      </div>
