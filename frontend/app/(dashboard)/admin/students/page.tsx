...
import { Edit2, Trash2, Download, RefreshCw, Plus, Sparkles, GraduationCap, FileSpreadsheet } from "lucide-react";
...
const StudentCreateEditModal = nextDynamic(() => import("@/components/modals/StudentCreateEditModal"), {
  ssr: false,
});
...
export default function AdminStudentsPage() {
  ...
  const studentsQuery = useApiQuery<{ items: Student[]; total_pages: number }>(
    ["admin-students", String(page), search, JSON.stringify(filters)],
    `/api/admin/students?page=${page}&page_size=10&search=${encodeURIComponent(search)}${Object.entries(filters).map(([k,v]) => `&${k}=${encodeURIComponent(String(v))}`).join("")}`,
    { method: "GET" }
  );

  const { data, isLoading } = (studentsQuery as any);
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newStudent) => {
      ...
      updateCachedLists((items) => [
        { id: Date.now(), name: newStudent.name, student_code: newStudent.student_code, class_name: newStudent.class_name, facial_data_encoded: false },
        ...items,
      ]);
    },
    onSuccess: () => {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: (ids: number[]) =>
      Promise.all(ids.map(id => axios.delete(`${apiBase}/api/admin/students/${id}`, { headers: authHeaders }))),
    onMutate: async (ids) => {
      await queryClient.cancelQueries({ queryKey: ["admin-students"] });
  ...
  const bulkExportMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      ...
      const rows = [
        ...
        ...selectedStudents.map((s: Student) => [s.id, s.name, s.student_code, s.class_name || "", s.facial_data_encoded ? "Oui" : "Non"]),
      ];
  ...
  const columns: ColumnDef<any>[] = [
    ...
    { header: "Classe", accessorKey: "class_name" },
    {
      header: "Visage",
      cell: ({ row }) => (
        <span className={`rounded px-2 py-0.5 text-xs ${row.original.facial_data_encoded ? "bg-emerald-700/40 text-emerald-300" : "bg-zinc-700/40 text-zinc-300"}`}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <button className="rounded p-1 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200" aria-label="Modifier">
      ...
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-amber-500/15 bg-gradient-to-br from-amber-500/10 via-zinc-950 to-black p-6">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-white/70">Filtrez les cohortes, suivez le statut facial et exportez en CSV en un clic.</p>
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[{
            ...
            label: "Manquants",
            value: Math.max((data?.items?.length ?? 0) - (data?.items?.filter((s: Student) => s.facial_data_encoded)?.length ?? 0), 0),
            tone: "text-red-200",
            bg: "bg-red-500/15"
          }].map((card) => (
        ...
        <OnboardingTour
          ...
          steps={[
            ...
            },
            {
              ...
              content: "Les mises à jour WebSocket s'appliquent automatiquement; forcez ici si besoin.",
              placement: "right",
            },
            {
