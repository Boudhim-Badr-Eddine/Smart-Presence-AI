...
const DataTable = nextDynamic(() => import("@/components/table/DataTable"), {
  ssr: false,
  loading: () => <div className="py-6"><TableSkeleton /></div>,
});

const SessionCreateEditModal = nextDynamic(() => import("@/components/modals/SessionCreateEditModal"), {
  ssr: false,
});
...
export default function AdminSessionsPage() {
  ...
  const updateCachedLists = (updater: (items: Session[]) => Session[]) => {
    const queries = queryClient.getQueriesData<{ items: Session[]; total_pages: number }>({ queryKey: ["admin-sessions"] });
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newSession) => {
      ...
      updateCachedLists((items) => [
        { id: Date.now(), title: newSession.title, date: newSession.date, time: newSession.time, class_name: newSession.class_name, trainer_name: newSession.trainer_name },
        ...items,
      ]);
    },
    onSuccess: () => {
  ...
  const handleExport = () => {
    const rows = [
      ...
      ...(data?.items.map((s: Session) => [s.title, s.date, s.time, s.class_name, s.trainer_name]) ?? []),
    ];
    ...
    <RoleGuard allow={["admin"]}>
      <div className="mx-auto max-w-7xl p-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-zinc-400">Planification et gestion des sessions: calendrier, affectations, présences.</p>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <AdvancedFilters
              filters={[
                ...
                { name: "session_type", type: "multiselect", label: "Type", options: [
                  { id: "theory", label: "Théorie" },
                  { id: "lab", label: "Atelier" },
                  { id: "exam", label: "Examen" },
                ] },
              ]}
              onApply={(f) => { setSearch(f.search ?? search); setDateRange({ from: f.from ?? dateRange.from, to: f.to ?? dateRange.to }); setPage(0); }}
              onReset={() => { setSearch(""); setDateRange({}); setPage(0); }}
