...
import { CheckCircle, XCircle, Clock, Search, Filter, Download, BarChart3, FileText, Eye, ThumbsUp, ThumbsDown } from "lucide-react";
...
export default function TrainerAttendancePage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${apiBase}/api/trainer/attendance`).catch(() => ({
        data: {
          items: [
            { id: 1, student: "Mohamed Alaoui", email: "mohamed@example.com", session: "Dev Web", status: "present", date: "2025-12-12", percentage: 100 },
            { id: 2, student: "Sara Bennani", email: "sara@example.com", session: "Dev Web", status: "absent", date: "2025-12-12", percentage: 0, justification: { status: "pending", reason: "Maladie, certificat envoyé.", submitted_at: "2025-12-12T07:30:00Z" } },
            { id: 3, student: "Youssef Amrani", email: "youssef@example.com", session: "Dev Web", status: "late", date: "2025-12-12", percentage: 75, justification: { status: "pending", reason: "Transport en grève", submitted_at: "2025-12-12T08:00:00Z" } },
            { id: 4, student: "Ahmed Ben Ali", email: "ahmed@example.com", session: "Database", status: "present", date: "2025-12-13", percentage: 100 },
            { id: 5, student: "Fatima Zahra", email: "fatima@example.com", session: "Database", status: "excused", date: "2025-12-13", percentage: 50, justification: { status: "approved", reason: "Déplacement administratif", submitted_at: "2025-12-11T18:00:00Z", admin_comment: "Document reçu" } },
            { id: 6, student: "Karim El Khabazi", email: "karim@example.com", session: "Dev Web", status: "present", date: "2025-12-14", percentage: 100 },
          ],
        },
      }));
  ...
  const pendingJustifications = attendanceRecords.filter((r) => r.justification?.status === "pending");

  const updateJustification = useMutation({
    mutationFn: async (payload: { id: number; status: "approved" | "rejected"; admin_comment?: string }) => {
    ...
    onMutate: async (payload) => {
      ...
      const previous = queryClient.getQueryData<{ items: AttendanceRecord[] }>(["trainer-attendance"]);
        ...
        items: (previous?.items ?? []).map((record) =>
          record.id === payload.id
            ? {
                ...record,
                justification: {
                  ...(record.justification ?? { reason: "", submitted_at: new Date().toISOString() }),
  ...
  return (
    <RoleGuard allow={["trainer"]}>
      <div className="mx-auto max-w-7xl p-6">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-white dark:text-white light:text-gray-900">Suivi des présences</h1>
          <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Consultez et gérez les présences des étudiants</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6 mb-6">
          {[
            ...
            { label: "Présents", value: stats.present, color: "bg-emerald-600/20 text-emerald-300", icon: CheckCircle },
            { label: "Absents", value: stats.absent, color: "bg-red-600/20 text-red-300", icon: XCircle },
            { label: "Retard", value: stats.late, color: "bg-amber-600/20 text-amber-300", icon: Clock },
            { label: "Excusés", value: stats.excused, color: "bg-blue-600/20 text-blue-300", icon: Clock },
            { label: "Justif. en attente", value: stats.pending, color: "bg-purple-600/20 text-purple-300", icon: FileText },
          ].map((stat, idx) => {
            const Icon = stat.icon;
            return (
              <div key={idx} className={`rounded-lg border border-white/10 ${stat.color} p-4 dark:border-white/10 light:border-gray-200`}>
        ...
        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 dark:border-white/10 light:border-gray-200">
            <div>
              <h3 className="text-lg font-semibold text-white dark:text-white light:text-gray-900">Justifications à examiner</h3>
              <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Absences/retards en attente d'approbation</p>
            </div>
            <span className="rounded-full bg-purple-600/20 px-3 py-1 text-xs font-medium text-purple-300">{pendingJustifications.length} en attente</span>
          </div>
              ...
              {pendingJustifications.map((record, idx) => (
                ...
                >
                  <div className="space-y-1">
                    <p className="font-semibold text-white dark:text-white light:text-gray-900">{record.student}</p>
                    <p className="text-xs text-zinc-400 dark:text-zinc-400 light:text-gray-600">{record.email} • {record.session} • {record.date}</p>
                    <div className="flex flex-wrap gap-2 text-xs text-zinc-300 dark:text-zinc-300 light:text-gray-700">
                      <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium ${getStatusColor(record.status)}`}>
                        {getStatusLabel(record.status)}
                      </span>
                      <span className="inline-flex items-center gap-1 rounded-full bg-purple-600/20 px-3 py-1 text-purple-200">
                        <FileText className="h-3 w-3" />
                        En attente depuis {new Date(record.justification?.submitted_at ?? record.date).toLocaleDateString('fr-FR')}
                      </span>
                    </div>
                    <p className="text-sm text-zinc-200 dark:text-zinc-200 light:text-gray-800">{record.justification?.reason}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => updateJustification.mutate({ id: record.id, status: "approved", admin_comment: "Justification acceptée" })}
                    ...
                    <button
                      onClick={() => updateJustification.mutate({ id: record.id, status: "rejected", admin_comment: "Pièce manquante" })}
          ...
          <div className="divide-y divide-white/5 dark:divide-white/5 light:divide-gray-200 max-h-96 overflow-y-auto">
            {filteredRecords.length > 0 ? (
              filteredRecords.map((record, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-medium text-white dark:text-white light:text-gray-900">{record.student}</p>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      {record.percentage !== undefined && (
                        <p className="text-xs text-zinc-400 dark:text-zinc-400 light:text-gray-600 mt-1">{record.percentage}%</p>
                      )}
                      {record.justification && (
                        <span className={`mt-2 inline-flex px-3 py-1 rounded-full text-xs font-medium ${getJustificationColor(record.justification.status)}`}>
              ...
              <div className="px-6 py-8 text-center">
                <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Aucun enregistrement trouvé</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-zinc-400">Détail justification</p>
                <h3 className="text-xl font-semibold text-white dark:text-white light:text-gray-900">{selectedRecord.student}</h3>
                <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">{selectedRecord.email}</p>
              </div>
              <button onClick={() => setSelectedRecord(null)} className="text-white/60 hover:text-white">✕</button>
            </div>

            <div className="space-y-3">
              <div className="flex items-center gap-3 text-sm text-zinc-300 dark:text-zinc-300 light:text-gray-700">
                <Clock className="h-4 w-4" />
                <span>{selectedRecord.session} • {selectedRecord.date}</span>
              </div>
              {selectedRecord.justification ? (
                <>
                  <div className="rounded-lg border border-white/10 bg-white/5 p-3 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-gray-50">
                    <p className="text-sm font-semibold text-white dark:text-white light:text-gray-900 mb-1">Motif</p>
                    <p className="text-sm text-zinc-200 dark:text-zinc-200 light:text-gray-800 whitespace-pre-line">{selectedRecord.justification.reason}</p>
                    <p className="mt-2 text-xs text-zinc-500">Soumis le {new Date(selectedRecord.justification.submitted_at).toLocaleString('fr-FR')}</p>
                  </div>
                  {selectedRecord.justification.admin_comment && (
                    <div className="rounded-lg border border-white/10 bg-white/5 p-3 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-gray-50">
                      <p className="text-sm font-semibold text-white dark:text-white light:text-gray-900 mb-1">Commentaire admin</p>
                      <p className="text-sm text-zinc-200 dark:text-zinc-200 light:text-gray-800">{selectedRecord.justification.admin_comment}</p>
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium ${getStatusColor(selectedRecord.status)}`}>
                      {getStatusLabel(selectedRecord.status)}
                    </span>
                    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium ${getJustificationColor(selectedRecord.justification.status)}`}>
                      {getJustificationLabel(selectedRecord.justification.status)}
                    </span>
                  </div>
                </>
              ) : (
                <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Aucune justification fournie.</p>
              )}
            </div>
            ...
            {selectedRecord.justification && selectedRecord.justification.status === "pending" && (
              <div className="mt-6 grid gap-2 md:grid-cols-2">
                <button
                  onClick={() => updateJustification.mutate({ id: selectedRecord.id, status: "approved", admin_comment: "Approuvé depuis la fiche" })}
                ...
                <button
                  onClick={() => updateJustification.mutate({ id: selectedRecord.id, status: "rejected", admin_comment: "Refusé depuis la fiche" })}
