...
import { Bell, Check, Trash2, AlertCircle, Info, CheckCircle, BookOpen, Calendar, MessageCircle } from "lucide-react";
...
export default function TrainerNotificationsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${apiBase}/api/trainer/notifications`, {
        params: { unread_only: filter === "unread" },
      }).catch(() => ({
        data: {
          items: [
            { id: 1, title: "Justification en attente", message: "Sara Bennani a soumis une justification pour Dev Web", type: "warning", read: false, created_at: new Date().toISOString() },
            { id: 2, title: "Nouveau message étudiant", message: "Karim El Khabazi a ajouté un commentaire sur la séance du 15/12", type: "info", read: false, created_at: new Date().toISOString() },
            { id: 3, title: "Session modifiée", message: "La session Base de Données est décalée à 14h00", type: "success", read: true, created_at: new Date().toISOString() },
          ],
        },
      }));
  ...
  const markReadMutation = useMutation({
    mutationFn: (id: number) => axios.patch(`${apiBase}/api/trainer/notifications/${id}`, { read: true }),
  ...
  return (
    <RoleGuard allow={["trainer"]}>
      <div className="mx-auto max-w-5xl p-6">
        <Breadcrumbs items={[{ label: "Tableau de bord formateur", href: "/trainer" }, { label: "Notifications" }]} />

        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white dark:text-white light:text-gray-900">Notifications</h1>
            <p className="text-sm text-zinc-400 dark:text-zinc-400 light:text-gray-600">Suivez les alertes liées à vos sessions et étudiants</p>
          </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-3">
          <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-4 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-white dark:text-white light:text-gray-900">{data?.items.length ?? 0}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-4 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
            <BookOpen className="h-5 w-5 text-amber-300" />
            <div>
              <p className="text-sm text-zinc-300 dark:text-zinc-300 light:text-gray-700">Justifications</p>
              <p className="text-xl font-semibold text-white dark:text-white light:text-gray-900">{data?.items.filter((n) => n.title.toLowerCase().includes("justification")).length ?? 0}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-4 dark:border-white/10 dark:bg-white/5 light:border-gray-200 light:bg-white">
            <MessageCircle className="h-5 w-5 text-emerald-300" />
            <div>
              <p className="text-sm text-zinc-300 dark:text-zinc-300 light:text-gray-700">Messages étudiants</p>
              <p className="text-xl font-semibold text-white dark:text-white light:text-gray-900">{data?.items.filter((n) => n.title.toLowerCase().includes("message")).length ?? 0}</p>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-xs text-zinc-400 dark:text-zinc-400 light:text-gray-500">
                      ...
                      {notification.read ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-600/20 px-3 py-1 text-emerald-200">Lu</span>
                      ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-blue-600/20 px-3 py-1 text-blue-200">Non lu</span>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
