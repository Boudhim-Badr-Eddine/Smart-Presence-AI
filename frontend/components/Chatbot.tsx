	...
	for (const ep of endpoints) {
		try {
			...
			const candidate: string | undefined =
				data.reply || data.answer || data.message || data.content;
			...
			<div ref={listRef} className="flex-1 space-y-3 overflow-y-auto p-4">
				{messages.length === 0 && (
					<div className="rounded border border-zinc-800 bg-zinc-900 p-4 text-sm text-zinc-300">
						...
						Je peux vous aider à consulter vos présences, absences, horaires et notifications. Posez votre question ci-dessous ou cliquez sur un prompt rapide pour commencer.
					</div>
				)}
				{messages.map((m) => (
