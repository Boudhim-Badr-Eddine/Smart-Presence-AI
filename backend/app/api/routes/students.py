...
from app.schemas.student import StudentListResponse, StudentOut, StudentUpdate
...
def list_students(
    ...
    if current_user.role not in ["admin", "trainer"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only admin/trainer can view students"
        )
...
def get_student(
    ...
    if current_user.role not in ["admin", "trainer", "student"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to view student details"
        )
    ...
    if current_user.role == "student":
        if student.user_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Cannot access other student's data"
            )
...
def delete_student(
    ...
    if current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only admin can delete students"
        )
