...
from app.schemas.auth import LoginRequest, FacialLoginRequest, EnrollFacialRequest, Token, MeResponse
from app.services import auth as auth_service
from app.services.facial import match_user_by_image, enroll_user_faces
...
def login_facial(payload: FacialLoginRequest, db: Session = Depends(get_db)):
    if not payload.image_base64:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Face image is required")
    user = db.query(User).filter(User.email == payload.email).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found")
    student = db.query(Student).filter(Student.user_id == user.id).first()
    if not student or not student.facial_data_encoded:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No enrolled facial data for user")
    import base64
...
def enroll_face(payload: EnrollFacialRequest, db: Session = Depends(get_db)):
    if not payload.requires_three_images():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Provide at least 3 images")
    import base64
    image_bytes_list = []
...
def me(current_user: User = Depends(auth_service.get_current_user)):
    return MeResponse(id=current_user.id, email=current_user.email, role=current_user.role, last_login=current_user.last_login)
